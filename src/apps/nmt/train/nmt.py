...
class NMT(nn.Module):
    ...
    def save(self, path: str):
        print('save model parameters to [%s]' % path, file=sys.stdout)
...
def train(args: Dict):
    ...
    if np.abs(uniform_init) > 0.:
        print('uniformly initialize parameters [-%f, +%f]' % (uniform_init, uniform_init), file=sys.stdout)
    ...
    while True:
        ...
        for src_sents, tgt_sents in batch_iter(train_data, batch_size=train_batch_size, shuffle=True):
            ...
            if train_iter % valid_niter == 0:
                ...
                print('validation: iter %d, dev. ppl %f' % (train_iter, dev_ppl), file=sys.stdout)
                ...
                if epoch == int(args['--max-epoch']):
                    print('reached maximum number of epochs!', file=sys.stdout)
                    exit(0)
...
def decode(args: Dict[str, str]):
    ...
    print(f"load test source sentences from [{args['TEST_SOURCE_FILE']}]", file=sys.stdout)
    test_data_src = read_corpus(args['TEST_SOURCE_FILE'], source='src')
    if args['TEST_TARGET_FILE']:
        print(f"load test target sentences from [{args['TEST_TARGET_FILE']}]", file=sys.stdout)
        test_data_tgt = read_corpus(args['TEST_TARGET_FILE'], source='tgt')

    print(f"load model from {args['MODEL_PATH']}", file=sys.stdout)
    ...
    if args['TEST_TARGET_FILE']:
        ...
        print(f'Corpus BLEU: {bleu_score}', file=sys.stdout)
