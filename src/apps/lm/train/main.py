...
args.save = "%s/W_bits=%d_A_bits=%d" % (args.model_dir, args.W_bits, args.A_bits)
...
if args.model == 'Transformer':
    ...
    model = model.RNNModel(args.model, ntokens, args.emsize, args.nhid, args.nlayers, args.dropout, args.tied, W_bits=args.W_bits, A_bits=args.A_bits).to(device)

criterion = nn.CrossEntropyLoss()

if args.pretrained is not None:
    print("Loading from pretrained: %s" % args.pretrained)
...
try:
    for epoch in range(1, args.epochs+1):
        ...
        logdata["valid_ppl"].append(math.exp(val_loss))
        logdata["test_ppl"].append(math.exp(test_loss))
