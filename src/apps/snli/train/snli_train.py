...
start = time.time()
best_dev_acc = -1
header = '  Time Epoch Iteration Progress    (%Epoch)   Loss   Dev/Loss     Accuracy  Dev/Accuracy Test/Accuracy'
...
for epoch in range(args.epochs):
    ...
    for batch_idx, batch in enumerate(train_iter):
        ...
        logdata["train_acc"].append(train_acc)
        ...
        if iterations % args.dev_every == 0:
            ...
            logdata["dev_acc"].append(dev_acc)
            ...
            logdata["test_acc"].append(test_acc)

            print(dev_log_template.format(time.time()-start,
                                          epoch, iterations, 1+batch_idx, len(train_iter),
                                          100. * (1+batch_idx) / len(train_iter), loss.item(), dev_loss.item(), train_acc, dev_acc, test_acc))
