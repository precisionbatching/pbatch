...
for epoch in range(num_epochs):
    ...
    for i, (images, labels) in enumerate(train_loader):
        ...
        images = images.reshape(-1, 28*28).to(device)
        #print(set(list(images.numpy().flatten())))
        labels = labels.to(device)
        ...
        if (i+1) % 100 == 0:
            ...
            for m in [x for x in model.modules() if type(x) == QLinear]:
                plt.cla()
                plt.hist(list(m.weight_values.flatten()), density=True, stacked=True, bins=100, alpha=.5, label="Weights")
                plt.hist(list(m.activation_values.flatten()), density=True, stacked=True, bins=100, alpha=.5, label="Activations")
                plt.xticks(fontsize=18)
                plt.yticks(fontsize=18)
                #plt.yscale('log', nonposy='clip')
                #plt.xscale('log')
                plt.legend(loc="best", fontsize=22)
                plt.savefig("%d.pdf" % m.layer_id)
